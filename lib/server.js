...
class WebThingServer {
  ...
  constructor(thing, port, sslOptions) {
    ...
    this.app.get('/actions', (req, res) => {
      const response = [];
      for (const name in this.thing.actions) {
        for (const action of this.thing.actions[name]) {
          response.push(action.asActionDescription());
        }
      }
    ...
    this.app.post('/actions', (req, res) => {
      ...
      for (const actionName in req.body) {
        const action = this.thing.performAction(actionName,
                                                req.body[actionName]);
    ...
    this.app.get('/actions/:actionName/:actionId', (req, res) => {
      ...
      if (!this.thing.actions.hasOwnProperty(actionName)) {
        res.status(404).end();
        return;
      }

      for (const action of this.thing.actions[actionName]) {
